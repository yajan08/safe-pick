...
class AdminUsersTab extends ConsumerStatefulWidget {
  ...
  static void showRegisterDriverSheet(BuildContext context) {
    showModalBottomSheet(
      context: context,
      isScrollControlled: true,
      backgroundColor: Colors.transparent,
      builder: (context) => const _RegisterDriverSheet(),
    );
  }
  ...
  void _openUserDetail(UserModel user) {
    if (user.role == 'driver') {
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => AdminDriverDetailScreen(driver: user),
        ),
      );
    } else if (user.role == 'parent') {
      Navigator.of(context).push(
        MaterialPageRoute(
          builder: (_) => AdminParentDetailScreen(parent: user),
        ),
      );
    } else {
      // Default / fallback
              ...
              return Padding(
                ...
                child: ChoiceChip(
                  ...
                  shape: RoundedRectangleBorder(
                    ...
                    side: BorderSide(
                      color: sel
                          ? kAdminNavy.withValues(alpha: 0.4)
              ...
              return ListView.builder(
                ...
                itemBuilder: (context, i) {
                  ...
                  return _UserTile(
                      ...
                      .animate()
                      .fade(delay: Duration(milliseconds: 25 * i.clamp(0, 10)))
                      .slideX(begin: 0.015);
                },
              );
            },
...
class _UserTile extends StatelessWidget {
  ...
  Color get _roleColor => user.role == 'driver'
      ? Colors.teal
      : user.role == 'admin'
      ? kAdminNavy
      : Colors.blueGrey;
  IconData get _roleIcon => user.role == 'driver'
...
class _UserDetailSheetState extends ConsumerState<_UserDetailSheet> {
  ...
  void initState() {
    ...
    _vehicleCtrl = TextEditingController(
      text: widget.user.vehicleNumbers.join(', '),
    );
  ...
  Future<void> _saveEdits() async {
    ...
    try {
      ...
      if (_phoneCtrl.text.trim() != widget.user.phone) {
        updates['phone'] = _phoneCtrl.text.trim();
      }
      if (widget.user.role == 'driver') {
        final vehicles = _vehicleCtrl.text
            .split(',')
            .map((v) => v.trim())
          ...
          children: [
            ...
            if (_isEditing) ...[
              ...
              if (widget.user.role == 'driver') ...[
                const SizedBox(height: 12),
                _EditField(
                  label: 'Vehicles (comma-separated)',
              ...
              _DetailRow(
                ...
                label: 'Phone',
                value: widget.user.phone.isNotEmpty ? widget.user.phone : '—',
              ),
              _DetailRow(
              ...
              if (widget.user.role == 'driver' &&
                  widget.user.vehicleNumbers.isNotEmpty)
                _DetailRow(
                  icon: Icons.directions_car_outlined,
                  label: 'Vehicles',
                  value: widget.user.vehicleNumbers.join(', '),
                ),
            ],
            ...
            if (widget.user.role == 'driver') ...[
              ...
              if (_isLoadingSessions)
                ...
                _DriverMetricRow(
                  ...
                  value: '${_driverSessions.length}',
                ),
                _DriverMetricRow(
                  label: 'Completed',
                  value:
                      '${_driverSessions.where((s) => s.status == 'completed').length}',
                ),
                _DriverMetricRow(
                  label: 'Active Vehicles',
                  value: widget.user.vehicleNumbers.isNotEmpty
                      ? widget.user.vehicleNumbers.join(', ')
                      : '—',
                ),
              ],
            ],
...
class _RegisterDriverSheet extends ConsumerStatefulWidget {
  const _RegisterDriverSheet();

  @override
  ConsumerState<_RegisterDriverSheet> createState() => _RegisterDriverSheetState();
}

class _RegisterDriverSheetState extends ConsumerState<_RegisterDriverSheet> {
  ...
  Future<void> _submit() async {
    ...
    try {
      await ref.read(auditServiceProvider).registerDriver(
          ...
          child: Column(
            ...
            children: [
              ...
              Row(
                children: [
                  CircleAvatar(
                    radius: 20,
                    backgroundColor: kAdminNavy.withValues(alpha: 0.08),
                    child: const Icon(Icons.person_add_rounded, color: kAdminNavy, size: 20),
                  ),
                  const SizedBox(width: 12),
                  const Text(
                    'Register New Driver',
              ...
              TextFormField(
                ...
                validator: (v) => v == null || v.trim().isEmpty ? 'Please enter driver\'s name' : null,
              ),
              const SizedBox(height: 16),
              ...
              TextFormField(
                ...
                decoration: InputDecoration(
                  ...
                  suffixIcon: IconButton(
                    icon: Icon(_obscurePass ? Icons.visibility_outlined : Icons.visibility_off_outlined, color: AppTheme.textMuted, size: 20),
